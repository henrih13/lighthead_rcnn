...
class Config:
    ...
    #weight = osp.join(root_dir, 'data/imagenet_weights/xception_model.ckpt')
    #weight = osp.join(root_dir, 'data/imagenet_weights/model.ckpt')
    #weight = osp.join(root_dir, 'data/imagenet_weights/mobilenet_v2_0.35_96.ckpt')
    ...
    from class_config import class_info
    ...
    #classes_originID = {'cpu': 1 }
    #class_num = 2 #datadb.num_classes
    #num_classes = 2 #datadb.num_classes
    #class_names = ['background', 'cpu'] #datadb.class_names
    class_info = class_info
    classes_originID = class_info.classes_originID
    class_num = class_info.num_classes
    ...
    batch_image_preprocess = 'pad'
    train_root_folder = os.path.join(root_dir, 'data/motherboard')
    train_source = os.path.join(
        root_dir, 'data', 'motherboard/odformat/mb_train.odgt')

    eval_root_folder = os.path.join(root_dir, 'data/motherboard')
    eval_source = os.path.join(
        root_dir, 'data', 'motherboard/odformat/mb_val.odgt')

    test_root_folder = os.path.join(root_dir, 'data/motherboard/test/img_to_test/')


    filter_gt_ignore_label = True
    train_gt_ignore_label = False

    image_short_size = 800 #800
    image_max_size = 1000 #1333
    eval_resize = True
    eval_image_short_size = 480 #800
    eval_image_max_size = 640 #1333

    test_max_boxes_per_image = 100 #100
    test_cls_threshold = 0.00
    test_vis_threshold = 0.5
    test_nms = 0.5
    test_save_type = 'motherboard'#'coco'

    batch_filter_box_size = 0
    max_boxes_of_image = 100 #100
    nr_box_dim = 5
    nr_info_dim = 6

    stride = [16]
    anchor_scales = [2, 4, 8, 16, 32]#[2, 4, 8, 16, 32]
    ...
    nr_image_per_epoch = 667 #80000  # detectron 1x setting
    ...
    multi_stage_lr_policy = lr_policy.MultiStageLR([[19, basic_lr], [25, basic_lr * 0.1], [30, basic_lr * 0.01]]) ##satt av light head rcnn at max er 30 epochs, ellers vil den gi 0.00/NonType i return
    ...
    TEST.RPN_POST_NMS_TOP_N = 100 ##1000 ##300 used other places
