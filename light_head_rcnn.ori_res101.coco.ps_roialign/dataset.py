...
def train_dataset(seed=config.seed_dataprovider):
    ...
    nr_files = len(files)
    #print(files+"sssssssssssssssssssssssssssssssss------------------sss")
    # shuffle the dataset
    np.random.seed(seed)
...
def test_dataset():
    ...
    def read_img(img_name):
        ...
        img = cv2.imread(image_path, cv2.IMREAD_COLOR)
        return dict(data=img, img_name=img_name, image_path=image_path)

    return dict(img=files, nr_records=total_files, read_img=read_img)
